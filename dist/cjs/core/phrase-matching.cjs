...
function findFuzzyPhrase(text, phrase, maxEditDistance, useTranspositions) {
  ...
  for (let i = 0; i <= textWords.length - phraseWords.length; i++) {
    ...
    for (let j = 0; j < phraseWords.length; j++) {
      const distance = useTranspositions ? levenshtein.calculateDamerauLevenshteinDistance(phraseWords[j], segment[j], maxEditDistance) : levenshtein.calculateLevenshteinDistance(phraseWords[j], segment[j], maxEditDistance);
...
//# sourceMappingURL=phrase-matching.cjs.map
