...
export class SpanishProcessor extends BaseLanguageProcessor {
    ...
    for (let i = 0; i < normalized.length; i++) {
      ...
      switch (char) {
        ...
        case 'f':
          ...
          break;
        case 'g':
          if (next === 'u' && i + 2 < normalized.length && 
              (normalized[i + 2] === 'e' || normalized[i + 2] === 'i')) {
            digit = '4'; // GUE, GUI
          } else if (next === 'e' || next === 'i') {
  ...
  protected getCommonEndings(): string[] {
    return [
      'o', 'a', 'os', 'as', 'e', 'es', 'ar', 'er', 'ir',
      'ado', 'ada', 'idos', 'idas', 'ando', 'endo', 'iendo',
      'cion', 'sion', 'dad', 'tad', 'mente', 'oso', 'osa',
      'ito', 'ita', 'illo', 'illa'
    ];
  }
  ...
  getSynonyms(word: string): string[] {
    const synonymMap: Record<string, string[]> = {
      'medico': ['doctor', 'facultativo'],
      'hospital': ['clinica', 'sanatorio'],
      'escuela': ['colegio', 'instituto'],
      'coche': ['auto', 'automovil', 'vehiculo'],
      'casa': ['hogar', 'vivienda', 'domicilio'],
      'calle': ['via', 'avenida', 'carretera'],
      'ciudad': ['urbe', 'poblacion', 'municipio'],
      'trabajo': ['empleo', 'ocupacion', 'labor'],
      'dinero': ['plata', 'efectivo', 'capital'],
      'tiempo': ['momento', 'periodo', 'duracion'],
      'grande': ['enorme', 'gigante', 'inmenso'],
      'pequeno': ['chico', 'diminuto', 'minusculo'],
      'rapido': ['veloz', 'ligero', 'acelerado'],
      'lento': ['despacio', 'pausado'],
      'bueno': ['excelente', 'magnifico', 'estupendo'],
      'malo': ['pesimo', 'terrible', 'horrible']
    };
